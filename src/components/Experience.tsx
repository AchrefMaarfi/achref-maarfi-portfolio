...
import { durationFromNow } from "@/lib/utils";
          ...
          <ol className="relative space-y-10 pl-10 before:absolute before:top-1 before:bottom-1 before:left-3 before:w-px before:bg-border">
            {experience.map((entry, i) => (
              <li key={`${entry.company}-${entry.start}`} className="relative">
                <span
                  aria-hidden="true"
                  className="border-background bg-primary shadow-glow-amber absolute top-1 -left-7 size-3.5 -translate-x-1/2 rounded-full border-2"
                />
                <Reveal delay={i * 80}>
                  <article>
                    <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                      <h3 className="text-lg font-bold">
                        {entry.positions ? entry.company : entry.role}
                      </h3>
                      <p className="text-muted-foreground text-xs font-medium tracking-wide uppercase">
                        {entry.start} – {entry.end}
                        {entry.end === "Present" && (
                          <> · {durationFromNow(entry.start)}</>
                        )}
                      </p>
                    </div>
                      ...
                      <ol className="mt-4 space-y-4">
                        {entry.positions.map((position) => (
                          <li key={position.title} className="relative pl-5">
                            <span
                              aria-hidden="true"
                              className="bg-muted-foreground/50 absolute top-1.5 left-0 size-1.5 rounded-full"
                            />
                            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-0.5">
                              ...
                              <p className="text-muted-foreground text-[0.7rem] font-medium tracking-wide uppercase">
                                ...
                                {position.end === "Present" && (
                                  <> · {durationFromNow(position.start)}</>
                                )}
                              </p>
                            </div>
                            ...
                            <ul className="text-muted-foreground mt-2 list-none space-y-1.5 text-sm leading-relaxed">
